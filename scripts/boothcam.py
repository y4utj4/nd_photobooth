...
def snap(can, countdown1, effect):
    global image_idx
...
#           camera.capture(custom.RAW_FILENAME[:-4] + '_4.' + custom.EXT, resize=(683, 384))
            ...
            snapshot = Image.new('RGBA', (1366, 768), (255,255,255,0) )
            snapshot.paste(Image.open(custom.RAW_FILENAME[:-4] + '_1.' + custom.EXT).resize((663, 364)), (  0,   0,  663, 364))
            snapshot.paste(Image.open(custom.RAW_FILENAME[:-4] + '_2.' + custom.EXT).resize((663, 364)), (683,   0, 1346, 364))
            snapshot.paste(Image.open(custom.RAW_FILENAME[:-4] + '_3.' + custom.EXT).resize((663, 364)), (  0, 384,  663, 748))
#           snapshot.paste(Image.open('custom.RAW_FILENAME[:-4] + '_4.' + custom.EXT').resize((663, 364)), (683, 384, 1346, 748))
            snapshot.paste(Image.open('./nd_pic4.jpg').resize((663, 364)), (683, 384, 1346, 748))
